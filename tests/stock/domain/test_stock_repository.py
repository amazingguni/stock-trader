...
def get_dummy_stock():
    return Stock(
        market=Stock.MARKET_KOSDAQ, name='우리회사', code='000000', sector='전자',
...
def test_save_or_modify_GIVEN_existing_stock(mongo_connection):
    ...
    # When
    new_stock = Stock(market=Stock.MARKET_KOSDAQ, name='우리식당', code='000000', sector='요식업',
...
def test_save_all(mongo_connection):
    ...
    for i in range(1, 11):
        stocks.append(
            Stock(market=Stock.MARKET_KOSDAQ, name=f'우리식당{i}', code=f'{i:06d}', sector='요식업',
                  major_product='양념치킨', listing_date=date(2018, 6, 16),
                  account_month='4월', region='용인'))
    # When
...
def test_find_all(mongo_connection):
    ...
    for i in range(1, 11):
        stocks.append(
            Stock(market=Stock.MARKET_KOSDAQ, name=f'우리식당{i}', code=f'{i:06d}',
...
def test_update(mongo_connection):
    Stock(market=Stock.MARKET_KOSDAQ, name='우리식당1', code='0001').save()
    Stock(market=Stock.MARKET_KOSDAQ, name='우리식당2', code='0002').save()
    Stock(market=Stock.MARKET_KOSDAQ, name='우리식당3', code='0003').save()
